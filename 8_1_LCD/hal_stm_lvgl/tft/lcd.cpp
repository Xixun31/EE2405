...
void LCD::FillRectangle(unsigned int x, unsigned int y, unsigned int w,
                        unsigned int h, uint16_t c) {

  int32_t act_x1 = x < 0 ? 0 : x;
	int32_t act_y1 = y < 0 ? 0 : y;
	int32_t act_x2 = x+w > TFT_HOR_RES - 1 ? TFT_HOR_RES - 1 : x+w;
	int32_t act_y2 = y+h > TFT_VER_RES - 1 ? TFT_VER_RES - 1 : y+h;
	x1_flush = act_x1;
	y1_flush = act_y1;
	x2_flush = act_x2;
	y2_flush = act_y2;
    ...
    temp_fb=(uint8_t*)my_fb;
    temp_fb=&temp_fb[2*(y_flush_act * TFT_HOR_RES + x1_flush)];
...
void LCD::LCD_Clear(uint16_t j) {
  FillRectangle(0, 0, TFT_HOR_RES, TFT_VER_RES, j);
}

void LCD::H_line(unsigned int x, unsigned int y, unsigned int w, uint16_t c) {
  FillRectangle(x, y, w, 1, c);
}

void LCD::V_line(unsigned int x, unsigned int y, unsigned int h, uint16_t c) {
  FillRectangle(x, y, 1, h, c);
}

void LCD::Rect(unsigned int x, unsigned int y, unsigned int w, unsigned int h, uint16_t c) {
  H_line(x, y, w, c);
	V_line(x, y, h, c);
  H_line(x, y+h, w, c);
	V_line(x+w, y, h, c);
