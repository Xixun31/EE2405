...
class GattServerDemo : ble::GattServer::EventHandler {
    ...
    const static uint16_t EXAMPLE_SERVICE_UUID         = 0xA600;
    const static uint16_t WRITABLE_CHARACTERISTIC_UUID = 0xA601;
...
public:
    ...
    void start(BLE &ble, events::EventQueue &event_queue)
    {
        ...
        printf("Example service added with UUID 0xA600\r\n");
        printf("Connect and write to characteristic 0xA601\r\n");
    }
...
private:
     ...
     */
    void blink_led_task(uint8_t count) {
    ...
    virtual void onDataWritten(const GattWriteCallbackParams &params)
    {
        if ((params.handle == _writable_characteristic->getValueHandle()) && (params.len == 1)) {
            ...
            uint8_t blink_count = *(params.data);
            extern EventQueue event_queue;
            event_queue.call(callback(this, &GattServerDemo::blink_led_task), blink_count);
        }
    }
