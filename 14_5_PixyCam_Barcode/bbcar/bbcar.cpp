    ...
    servo0(pinc_servo0, pinf_servo0), servo1(pinc_servo1, pinf_servo1)
{
    servo0.set_speed(0);
    servo1.set_speed(0);
    servo_control_ticker.attach(callback(this, &BBCar::controlWheel), 10ms);
...
void BBCar::driveLR( double left_speed, double right_speed ){
    servo0.set_factor(1);
    servo1.set_factor(1);
    // Convert left/right forward wheel commands to servo PWM sign convention.
